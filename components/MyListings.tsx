...
const MyListings: React.FC<MyListingsProps> = ({ items, onDelete }) => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to permanently remove this listing? This action will delete it from the campus hub.')) return;
    setIsDeleting(id);
    try {
      await onDelete(id);
    } catch (err) {
            ...
            activeItems.map(item => (
              <div key={item.id} className="bg-white rounded-[2rem] border border-gray-100 p-4 shadow-sm flex gap-4 animate-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex-grow min-w-0 flex flex-col justify-between py-1">
                  ...
                  >
                    {isDeleting === item.id ? <div className="w-3 h-3 border-2 border-red-200 border-t-red-600 rounded-full animate-spin" /> : 'Delete Listing'}
                  </button>
                </div>
              </div>
            ))
